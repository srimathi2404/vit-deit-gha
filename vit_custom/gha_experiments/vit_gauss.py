...
class GraphHeadAttention(Module):
    ...
    def _create_distance_bias(self, grid_size, sigma):
        """
        Create (1, 1, N, N) bias including CLS token
        """
        coords = torch.stack(torch.meshgrid(
            torch.arange(grid_size),
            torch.arange(grid_size),
            indexing='ij'
        ), dim=-1).reshape(-1, 2).float()   # (num_patches, 2)

        # pairwise squared distance
        rel = coords[:, None, :] - coords[None, :, :]
        dist = (rel ** 2).sum(-1)  # (N, N)

        bias = -dist / (2 * sigma ** 2)  # Gaussian
        ...
        return full_bias.unsqueeze(0).unsqueeze(0)  # (1,1,N,N)

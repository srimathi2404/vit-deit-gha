...
class GraphHeadAttention(nn.Module):
    """
    Hybrid GHA:
    - Graph built from attention (paper idea)
    - Clean GAT message passing (stable)
    """
    ...
    def forward(self, x, attn_mask=None, **kwargs):
        ...
        adj = torch.zeros_like(attn)
        adj.scatter_(-1, topk_idx, 1.0)  # binary adjacency
        ...
        # Graph Attention (GAT)
        # -------------------------
        h = self.W(v)  # feature transform

        h_i = h.unsqueeze(-2)  # (B, H, N, 1, D)
        h_j = h.unsqueeze(-3)  # (B, H, 1, N, D)

        # concatenate features
        h_i_expand = h_i.expand(-1, -1, -1, N, -1)
        h_j_expand = h_j.expand(-1, -1, N, -1, -1)

        concat = torch.cat([h_i_expand, h_j_expand], dim=-1)

        # compute edge scores
        e = (concat * self.a.unsqueeze(0).unsqueeze(2).unsqueeze(3)).sum(dim=-1)
        e = self.leaky_relu(e)
